...
The ModelManager class is perhaps the most exceptional class, in its size showed in the Package Map as well as in its source code. The model manager keeps track of all the famix entities in HashMap datastructures, so it contains many attributes as well as getter and setter methods for those attributes. 

This class is also labeled as a God Class by the inFusion tool.
